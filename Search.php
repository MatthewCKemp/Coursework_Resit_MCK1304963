        ...
        if($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (mysqli_num_rows($result) > 0 ){
                ...
                while ($rows = mysqli_fetch_assoc($result)) { //Outputs data in each row.
                    echo "<tr><td><a href=template.php?bugID='$rows[bug_ID]'>" . $rows["title"] . "</a></td><td>" . $rows["bugposted"] . "</td><td><a href=UserTemplate.php?devID='$rows[user_ID]&name=$rows[name]'>" . $rows["name"] . "</a></tr></td>";
                }
                echo"</table>";
            } else {
