...
<div id="content">
    <div id="container">
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $Search = "SELECT Users.user_ID, Bugs.bug_ID, Bugs.title, Bugs.bugposted, Users.name FROM Bugs, Users WHERE Users.user_ID LIKE Bugs.user_ID AND Users.name LIKE '$Dev' AND Bugs.title LIKE '%". $Key ."%'";
            ...
            if (mysqli_num_rows($result) > 0 ){
                ...
                while ($rows = mysqli_fetch_assoc($result)) { //Outputs data in each row.
                    echo "<tr><td><a href=template.php?bugID=$rows[bug_ID]&title=$rows[title]>" . $rows["title"] . "</a></td><td>" . $rows["bugposted"] . "</td><td><a href=UserTemplate.php?devID=$rows[user_ID]&name=$rows[name]>" . $rows["name"] . "</a></tr></td>";
                }
                echo"</table>";
            } else {
